...
read container_path
echo ""
echo "6. Path to atlas folder"
echo "(Enter an answer)"
read atlas_path
echo ""
echo "7. Paths for slurm job notice emails and out files"
...

{
    ...
    echo "# SETTINGS"
    echo "######################################################"
    echo "# 7. Organism being imaged (human, mouse, or rat)"
    echo "organism=${organism}"
    echo ""
    echo "# 8. Diffusion metrics to generate (comma separated; options include: FA,MD,AD,RD,QA,ODI,NDI,MK)"
    echo "volumes=${volumes}"
    echo ""
    echo "# 9. Should MP-PCA denoising be applied to the diffusion image?"
    echo "do_mppca=1"
    echo ""
    echo "# 10. Should the 'bad slice removal' script be applied to the diffusion image?"
    echo "do_slice=0"
    echo ""
    echo "######################################################"
    echo "# MISCELLANEOUS"
    ...
    echo "special_ids=X"
    echo ""
    echo "# 11. Path to MDT singularity container (if needed)"
    echo "mdt_container_path=/path/to/mdt_container.sif"
    echo ""
    echo "######################################################"
    echo "######################################################"
} > "$filename"
