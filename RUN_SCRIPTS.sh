...
if [[ "$step" == "fit" ]]; then
    ...
    output_dir="${reg_output_dir}/roi_dist_output/"
    mkdir $output_dir
    cd ${reg_output_dir}
...
elif [[ "$step" == "vba_avg" || "$step" == "vba_compare" || "$step" == "vba_correlate" ]]; then
    #########################################
    ...
    if [[ "$step" == "vba_correlate" ]]
    then
        mkdir -p  "${reg_output_dir}/vba/correlations/"
        if [[ "$design" == "baseline" ]]
        then
            echo "[$(date '+%Y-%m-%d %H:%M:%S')] Voxel-based analysis - correlating groups to independent variable using a BASELINE (single timepoint, single group) design"
            for vol in "${volumes[@]}"
            do
                echo "[$(date '+%Y-%m-%d %H:%M:%S')] Making stat maps for ${vol}"
                singularity exec $container_path /opt/conda/envs/diffusionmritoolkit/bin/python ${scripts_dir}/ODTB-statmaps_corr.py \
                    --rootdir $reg_output_dir \
                    --mask $mask_path \
                    --outdir "${reg_output_dir}/vba/correlations/" \
                    --avg_vol_dir "${reg_output_dir}/vba/avg_volumes/" \
                    --data $corr_data_f \
                    --organism $organism \
                    --vt ${vol} \
            ...
            echo "vba_correlate only currently supported with --design=baseline"
            exit 1
        fi
...
elif [[ "$step" == "origspace" ]]; then
    #########################################
    # misc - ORIGINAL SPACE STATS
    #########################################
    ...
    subdirs=$(ls)
    n_subdirs=`find . -mindepth 1 -maxdepth 1 -type d | wc -l`
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] Identified $n_subdirs subdirectories in root directory for original space statistics in $rootdir"
    echo " "
    count=1
...
elif [[ "$step" == "reverse_warp_mask" ]]; then
    #########################################
    # misc - ORIGINAL SPACE STATS
    #########################################
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] Reverse-warping mask"
    ...
    subdirs=$(ls)
    n_subdirs=`find . -mindepth 1 -maxdepth 1 -type d | wc -l`
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] Identified $n_subdirs subdirectories in root directory for reverse-warping in $rootdir"
    echo " "
    count=1
