...
miracl_sing_path="/scratch/groups/rairan/NODDI/MIRACL/miracl_latest.sif"
dsi_sing_path="/scratch/groups/rairan/DSI/dsistudio_latest.sif"
...
singularity exec $miracl_sing_path python /home/groups/rairan/NODDI/experimental/sne-dsi.py $FA ${out_dir}/atlas_warp_flirt_RPI.nii.gz ${out_dir}
...
for ((i=1; i<=max_int; i++)); do
    fslmaths ${out_dir}/atlas_warp_abbrev.nii.gz -thr ${i} -uthr ${i} "${out_dir}/roi_${i}.nii.gz"
    singularity exec $dsi_sing_path dsi_studio --action=ana \
        --source=${out_dir}/${out_fname}_DSI.fib.gz \
        --tract=${out_dir}/${out_fname}_DSI.tt.gz \
